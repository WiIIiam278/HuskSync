...
public interface BukkitUserDataHolder extends UserDataHolder {
    ...
    default Optional<Data.Items.Inventory> getInventory() {
        if (isDead() && getPlugin().getSettings().getSynchronization().getSaveOnDeath().isEnabled()
            && !getPlugin().getSettings().getSynchronization().getSaveOnDeath().isSyncDeadPlayersChangingServer()) {
